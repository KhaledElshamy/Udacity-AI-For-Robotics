...
def sense(p, colors, measurement, sensor_right):
    ...
    s = sum(sum(inner_array) for inner_array in q)
...
def move(p, motion, p_move):
    ...
    for x in range(len(q[0])):
        for y in range(len(q)):
            s = p_move * p[(y-motion[0]) % len(p)][(x-motion[1]) % len(p[0])]
            s += p_stay * p[y][x]
            q[y][x] = s
    return q
...
def localize(colors,measurements,motions,sensor_right,p_move):
    ...
    for motion, measurement in zip(motions, measurements):
        # Update the position based on motion
        p = move(p, motion, p_move)

        # Update the probabilities based on measurement
