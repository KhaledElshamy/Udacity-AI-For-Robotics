...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # # Make sure the steering angle is within the limits
        steering = max(-max_steering_angle, min(max_steering_angle, steering))
        
        theta = self.orientation
        beta = distance/self.length*tan(steering) # turning angle 


        # Update the robot's position and orientation
        if abs(beta) > 0.001:
            turning_radius = distance / beta  # Compute the turning radius
            cx = self.x - sin(theta) * turning_radius
            cy = self.y + cos(theta) * turning_radius
            new_orientation = (theta + beta) % (2 * pi)
            new_x = cx + sin(new_orientation) * turning_radius
            new_y = cy - cos(new_orientation) * turning_radius
        else:
            new_x = self.x + distance * cos(theta)
            new_y = self.y + distance * sin(theta)

        new_x += random.gauss(0, self.distance_noise)
        new_y += random.gauss(0, self.distance_noise)
        new_orientation += random.gauss(0, self.steering_noise)
        new_orientation = new_orientation % (2*pi)
        
        # Create a new instance of the robot class with the updated coordinates
    ...
    def sense(self,add_noise=1): 
        ...
        for landmark in landmarks:
            ...
            res = (atan2(dy,dx) - self.orientation) % (2*pi)
            if add_noise:
                res += random.gauss(0,self.bearing_noise)
            Z.append(res)
        return Z
